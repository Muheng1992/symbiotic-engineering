...
```
[architect] → [implementer x N] → [tester] → [reviewer] → [integrator]
                                                    ↓ (issues found)
                                              [implementer] → [reviewer] (re-review)
```
...
6. Integrator merges and verifies

## Pattern 2: Parallel Review Swarm

Best for: Thorough review of critical code changes.

```
                    ┌→ [reviewer: quality]     ─┐
[implementer] →     ├→ [reviewer: performance] ─┤→ [tech-lead: synthesize]
                    ├→ [security-auditor]       ─┤
                    └→ [tester]                 ─┘
```
...
```
[Explore agents x 3] → [architect] → [implementer] → [tester + reviewer]
```
...
4. Testing and review in parallel
...
```
[architect: interface] → [tester: write tests] → [implementer: make tests pass] → [reviewer]
```
...
## Pattern 5: Full Army Deployment
...
Phase 3: [tester x 2] + [reviewer] + [security]      ─── parallel
Phase 4: [implementer] fixes from Phase 3             ─── sequential
Phase 5: [integrator] merges + final verification     ─── sequential
Phase 6: [documenter] + [reporter] + [doc-manager]   ─── parallel
```
...
```
[implementer] → [reviewer] → [implementer: fix] → [reviewer: re-review]
     ↑                                                      |
     └──────────── (until APPROVED) ────────────────────────┘
```
...
4. Tests verify documented behavior
...
### Don't: Skip the review loop
Unreviewed code = tech debt.
DO: Always run reviewer before integration.
